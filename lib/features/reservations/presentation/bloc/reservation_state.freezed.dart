...
as double?,productQuantities: null == productQuantities ? _self.productQuantities : productQuantities // ignore: cast_nullable_to_non_nullable
as Map<String, int>,
  ));
}
/// Create a copy of ReservationState
...
class _ReservationState implements ReservationState {
  const _ReservationState({this.status = ReservationStatus.initial, this.currentPhase = 1, this.reservation = const Reservation(tenantId: ''), this.professionalSelected = false, this.isDiscountActive = false, final  List<DateTime> availableSlots = const <DateTime>[], this.slotsStatus = SlotsStatus.initial, this.selectedSlotUtc, this.effectiveBarberId, this.idempotencyKey, this.errorMessage, final  List<CustomerCoupon> availableCoupons = const <CustomerCoupon>[], this.selectedCoupon, this.couponDiscount = 0, this.finalPrice, final  Map<String, int> productQuantities = const <String, int>{}}): _availableSlots = availableSlots,_availableCoupons = availableCoupons,_productQuantities = productQuantities;
...
@override@JsonKey() Map<String, int> get productQuantities {
  if (_productQuantities is EqualUnmodifiableMapView) return _productQuantities;
  // ignore: implicit_dynamic_type
...
as CustomerCoupon?,couponDiscount: null == couponDiscount ? _self.couponDiscount : couponDiscount // ignore: cast_nullable_to_non_nullable
as double,finalPrice: freezed == finalPrice ? _self.finalPrice : finalPrice // ignore: cast_nullable_to_non_nullable
as double?,productQuantities: null == productQuantities ? _self._productQuantities : productQuantities // ignore: cast_nullable_to_non_nullable
as Map<String, int>,
  ));
}
